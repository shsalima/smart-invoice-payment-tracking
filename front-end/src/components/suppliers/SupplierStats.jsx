...
export default function SupplierStats() {
    ...
    useEffect(() => {
        async function fetchSupplierStats() {
            try {
                const response = await axios.get(`${VITE_API_URL}/suppliers/${supplierId}/stats`, {
                    headers: {
            ...
            {hasInvoices ? (
                <>
                    <div className="text-center mb-8">
                        <p className="text-4xl font-serif font-bold text-gray-900">{formatCurrency(totalInvoiced)}</p>
                        <p className="text-[10px] text-gray-400 uppercase tracking-widest mt-1 font-bold">Total Invoiced</p>
                    </div>

                    <div className="grid grid-cols-2 gap-4 mb-8">
                        <div className="bg-emerald-50 p-4 rounded-xl text-center">
                            <p className="text-emerald-700 font-bold">{formatCurrency(totalPaid)}</p>
                            <p className="text-[10px] text-emerald-600 font-medium">Paid</p>
                        </div>
                        <div className="bg-red-50 p-4 rounded-xl text-center">
                            <p className="text-red-700 font-bold">{formatCurrency(totalOutstanding)}</p>
                    ...
                    <div className="space-y-4">
                        <div>
                            <div className="flex justify-between text-[10px] font-bold mb-2">
                                ...
                                <span className="text-gray-900">{paymentRate}%</span>
                            </div>
                ...
                <div className="text-center py-8 text-gray-400 text-sm italic border-t border-gray-50">
                    Aucune donnée statistique disponible pour ce fournisseur (Pas de factures).
                </div>
            )}
        </div>
    );
